...
<script setup lang="ts">
  ...
  import { fetchGetCollectionList, fetchDeleteCollection, fetchBatchDeleteCollections } from '@/api/system-manage'
  import { ElMessageBox } from 'element-plus'

  defineOptions({ name: 'CollectionManage' })
    ...
    core: {
      ...
      columnsFactory: () => [
        ...
        { prop: 'created_at', label: '创建时间', minWidth: 180, sortable: true },
        {
          ...
          width: 80,
          fixed: 'right',
          formatter: (row: Api.Admin.Collection) =>
  ...
  const handleSelectionChange = (selection: Api.Admin.Collection[]) => {
  ...
  const handleDelete = (id: number) => {
    ElMessageBox.confirm('确定要删除该收藏记录吗？', '删除确认', {
      ...
      await fetchDeleteCollection(id)
      refreshRemove()
    })
  }
  ...
  const handleBatchDelete = () => {
    ElMessageBox.confirm(
      `确定要删除选中的 ${selectedIds.value.length} 条收藏记录吗？`,
      '批量删除确认',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      ...
      await fetchBatchDeleteCollections(selectedIds.value)
      refreshRemove()
      selectedIds.value = []
    })
  }
