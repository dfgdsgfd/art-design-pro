...
<script setup lang="ts">
  ...
  import { fetchGetLikeList, fetchDeleteLike, fetchBatchDeleteLikes } from '@/api/system-manage'
  ...
  const handleSelectionChange = (selection: Api.Admin.Like[]) => {
  ...
  const handleBatchDelete = () => {
    ElMessageBox.confirm(
      `确定要删除选中的 ${selectedIds.value.length} 条点赞记录吗？`,
      '批量删除确认',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      ...
      await fetchBatchDeleteLikes(selectedIds.value)
      refreshRemove()
      selectedIds.value = []
    })
  }
